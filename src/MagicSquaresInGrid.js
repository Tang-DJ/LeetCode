...
//[[5,6,4,4,9,5,2],[8,3,9,9,8,1,3],[6,2,5,7,4,7,6],[8,6,5,3,5,1,6],[2,4,3,3,6,3,8],[3,8,2,6,2,5,1],[5,2,9,7,9,7,4]] 逻辑判断不全 答案1
...
var numMagicSquaresInside = function(grid) {
    ...
    for(var row = 0;row<grid.length-2;row++){
        for(var col = 0;col<grid.length-2;col++){
            if(isSame([grid[row][col],grid[row+1][col],grid[row+2][col],
                    grid[row][col+1],grid[row+1][col+1],grid[row+2][col+1],
                    grid[row][col+2],grid[row+2][col+2],grid[row+2][col+2]]))
                sum++;
        }
    }
    return sum;
...
var grid = [[4,3,8,4],[9,5,1,9],[2,7,6,2]]
